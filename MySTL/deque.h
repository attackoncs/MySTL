...
struct __deque_iterator : public iterator<random_access_iterator_tag, T> {
            ...
            try
            {
                ...
                else
                {
                    mystl::uninitialized_fill(mystl::
                                                  uninitialized_copy(start_, position, new_start),
                                              start_, value_copy);
                    start_ = new_start;
    ...
    void deque<T, Alloc, BufSiz>::__reallocate_map(size_type node_to_add, bool add_at_front)
    {
        ...
        {
            new_start = map_ + static_cast<difference_type>((map_size_ - new_nodes_num) / 2) + static_cast<difference_type>(add_at_front ? node_to_add : 0);
        ...
        else
        {
            ...
            new_start = new_map + static_cast<difference_type>((new_map_size - new_nodes_num) / 2) + static_cast<difference_type>(add_at_front ? node_to_add : 0);
            mystl::copy(start_.node, finish_.node + 1, new_start);
            map_allocator::deallocate(map_, map_size_);
            map_ = new_map;
