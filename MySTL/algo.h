...
namespace mystl
{
                            ...
                            ForwardIterator2 first2, ForwardIterator2 last2)
    {
        ...
        if (d1 < d2)
            return last1; // 若序列二长度大于序列一，不可能成为其子序列
                                               ...
                                               T pivot)
    {
        ...
        {
            while (*first < pivot)
                ++first; // 从左边找第一个大于 pivot 的元素
            --last;
            while (pivot < *last)
                --last; // 从右边找第一个小于 pivot 的元素
            if (!(first < last))
                return first;              // 交错，循环结束
